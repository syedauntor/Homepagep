/* PrintAndUse - Division Generator */
(function () {
    'use strict';
    ...
    function generateProblems() {
        const problems = [];
        let maxDivisor;
        if (state.difficulty === 'simple')      maxDivisor = 5;
        else if (state.difficulty === 'medium') maxDivisor = 10;
        else                                    maxDivisor = 12;
        ...
        for (let i = 0; i < state.numProblems; i++) {
            ...
            problems.push({ num1: dividend, num2: divisor, answer: quotient });
        }
    ...
    function renderPreview() {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 0; c < 2; c++) {
                ...
                cols += `<div style="width:33.33%;text-align:left"><div style="display:flex;align-items:center;gap:10px;font-size:20px">${num}<span>${p.num1}</span><span>&divide;</span><span>${p.num2}</span><span>=</span>${ans}</div></div>`;
            }
    ...
    function openPrintWindow(showAnswers) {
        ...
        const rowCount = Math.ceil(state.problems.length / 2);
        let problemsHtml = '';
    ...
    window.puDivGen = {
        ...
        reset: function() {
            const el=getEl('gen-title'); if(el) el.value='Division Worksheet';
            const el2=getEl('gen-difficulty'); if(el2) el2.value='simple';
            const el3=getEl('gen-num-problems'); if(el3) el3.value='20';
    ...
    document.querySelectorAll('#gen-title,#gen-difficulty,#gen-num-problems,#gen-show-number,#gen-show-name,#gen-show-date').forEach(el => {
