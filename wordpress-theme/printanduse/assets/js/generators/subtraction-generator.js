/* PrintAndUse - Subtraction Generator */
(function () {
    'use strict';
    ...
    function syncState() {
        const fields = { 'gen-title':'title','gen-difficulty':'difficulty','gen-orientation':'orientation' };
        Object.entries(fields).forEach(([id,key]) => { const el = getEl(id); if(el) state[key] = el.value; });
        const numEl = getEl('gen-num-problems'); if(numEl) state.numProblems = Math.min(20,Math.max(1,parseInt(numEl.value)||20));
        ['show-number','show-name','show-date'].forEach(s => { const el = getEl('gen-'+s); if(el) state[s.replace('-','').replace('show','show').replace('number','ProblemNumber').replace('name','Name').replace('date','Date')] = el.checked; });
    ...
    window.puSubGen = {
        ...
        reset: function() {
            ['gen-title','gen-difficulty','gen-orientation','gen-num-problems'].forEach(id => { const el=getEl(id); if(el) el.value=id==='gen-title'?'Subtraction Worksheet':id==='gen-difficulty'?'simple':id==='gen-orientation'?'horizontal':'20'; });
            ['gen-show-number','gen-show-name','gen-show-date'].forEach(id => { const el=getEl(id); if(el) el.checked=true; });
